...
project, a self-hosted Nancy ( http://nancyfx.org ) based web service
...
Quickstart
--------------
See below for the complete application.
...
`Description` parameters will appear in the Service Control Manager.
...
The generic type argument should be the descendent class of `ServiceShell`. `StartService`
will inspect that type to retrieve the `ServiceName` attribute.

The first argument is the start-up action. You can pass an anonymous function,
...
This will check for -i, -install, -u, -uninstall command line arguments and 
perform the installation or uninstallation. These should be run as Administrator.
